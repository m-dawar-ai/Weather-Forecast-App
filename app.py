                ...
                for item in forecast_data["list"][:5]:
                    st.write(
                        ...
                        **{item['dt_txt']}**
                        ...
                        🌡️ Temperature: {item['main']['temp']} °C

                        🌥️ Condition: {item['weather'][0]['description'].title()}
                        """
                    )

            else:
                st.error(
                    f"Forecast Error: {forecast_data.get('message', 'Unknown error')}"
                )

        else:
            st.error(
                f"Weather Error: {weather_data.get('message', 'Unknown error')}"
            )
